...
def sub_matrix(x, y, r):
    return matrix[
        max(0, y - r) : min(height, y + r + 1),
        max(0, x - r) : min(width, x + r + 1),
    ]
...
while True:
    ...
    removed = [(y, x) for x, y in zip(xs, ys) if check(x, y)]
    if (added := len(removed)) == 0:
        break
    total += added
    removed = np.transpose(removed)
    matrix[*removed] = False

print(f"totoal:{total}")
